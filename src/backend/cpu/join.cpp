...
#define PUSHBACKJOIN(Z, INT, VECTOR) VECTOR.push_back(kernel::join<T, INT>);
...
Array<T> join(const int dim, const std::vector<Array<T>> &inputs)
{
    ...
    static std::vector<std::function<void(const int, Array<T>, const std::vector< Array<T> >)>>  join_arr;
    if (join_arr.size() == 0) {
        BOOST_PP_REPEAT(200, PUSHBACKJOIN, join_arr)
    }
    getQueue().enqueue(join_arr[n_arrays], dim, out, inputs);
